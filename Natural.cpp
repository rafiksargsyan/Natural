...
Natural::Natural(const char* m, size_t length)
{
    ...
    if (l == 0) {
        ...
        size_t j = l - 1;
        this->length = (j - i + POWER) / POWER;
        n = new Block[this->length];
        size_t k = this->length - 1;
        while (j > i + POWER - 1) {
            n[k--] = stringToUInt(m, j - POWER + 1, j);
            j -= POWER;
        }
...
std::istream& operator>>(std::istream& in, Natural& n)
{
    ...
    if (m.length() == 0) {
        ...
        size_t j = m.length() - 1;
        n.length = (j - i + POWER) / POWER;
        n.n = new Block[n.length]; 
        size_t k = n.length - 1;
        while (j > i + POWER - 1) {
            n.n[k--] = Natural::stringToUInt(m, j - POWER + 1, j);
            j -= POWER;
        }
